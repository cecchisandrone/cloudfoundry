...
public class PersonForm extends Form implements ClickListener, Receiver {
	...
	public PersonForm(PersonContainer personContainer) {

		this.personContainer = personContainer;
		/*
		 * Enable buffering so that commit() must be called for the form before input is written to the data. (Form input is not written immediately through to the underlying object.)
		 */
		...
		for (Iterator<Person> it = personContainer.getItemIds().iterator(); it.hasNext();) {
	...
	public void buttonClick(ClickEvent event) {
		...
		if (source == save) {
			...
			if (newContactMode) {
				...
				Item addedItem = personContainer.addItem(newPerson);
				/*
				 * We must update the form to use the Item from our datasource as we are now in edit mode (no longer in add mode)
				 */
			...
			try {
				...
				getApplication().getMainWindow().showNotification("Oops, someone else modified the data " + "simultaneously, refreshing data", Notification.TYPE_ERROR_MESSAGE);
				PersonContainer c = ((AddressBookApplication) getApplication()).getPersonContainer();
				c.populateContainer();
			}
			setReadOnly(true);
